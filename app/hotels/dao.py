    ...
    async def find_by_location(cls, location: str, date_from: date, date_to: date):
        async with async_session_maker() as session:
            ...
            rooms_quantity = select(
                ...
                Hotels.location,
                Hotels.services,
                (Hotels.rooms_quantity - func.coalesce(select(booked_rooms.c.booked_count).scalar_subquery(), 0)).label(
                    "rooms_quantity"),
                Hotels.image_id
    ...
    async def get_list_rooms(cls, hotel_id, date_from: date, date_to: date):
        async with (async_session_maker() as session):
            booked_rooms = select(Bookings.total_days, func.count('*').label('total'
                                 )).select_from(Bookings).join(Rooms,Rooms.id == Bookings.room_id).where(
            ...
            list_rooms = select(
                ...
                func.coalesce(Rooms.price * select(booked_rooms.c.total_days).scalar_subquery(),Rooms.price).label('total_cost'),
                func.coalesce(Rooms.quantity - select(booked_rooms.c.total).scalar_subquery(),Rooms.quantity).label('rooms_left')
...
#
#
# asyncio.run(example_list_rooms())
